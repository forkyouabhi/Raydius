...
export const requestOtp = async (req, res) => {
  ...
  // 2. Check if the university domain is allowed
  ...
  // 4. Save user and OTP hash to database
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: 'Your Raydius Login Code',
...
export const verifyOtp = async (req, res) => {
  ...
  if (!user || !user.otpHash || !user.otpExpiresAt) {
    return res.status(400).json({ error: 'Invalid request. Please request a code first.' });
  }
  ...
  // 3. OTP is valid, update user and create JWT
  user.verified = true;
